...
contains
  ...
  subroutine plot_interpolate_bands
    ...
    use w90_parameters, only: num_wann, bands_num_points, recip_metric, &
      ...
      bands_kpt_explicit, bands_kpt_frac, &
    ...
    if (.not. bands_kpt_explicit) then
      allocate (kpath_len(bands_num_spec_points/2), stat=ierr)
      if (ierr /= 0) call io_error('Error in allocating kpath_len in plot_interpolate_bands')
      allocate (kpath_pts(bands_num_spec_points/2), stat=ierr)
      if (ierr /= 0) call io_error('Error in allocating kpath_pts in plot_interpolate_bands')
      allocate (kpath_print_first_point(bands_num_spec_points/2), stat=ierr)
      if (ierr /= 0) call io_error('Error in allocating kpath_print_first_point in plot_interpolate_bands')
    ...
    if (bands_kpt_explicit) then
      total_pts = size(bands_kpt_frac, 2)

      ! Count the total number of special points
      num_spts = 0
      do i = 1, total_pts
        do j = 1, bands_num_spec_points
          if (sum((bands_kpt_frac(:, i) - bands_spec_points(:, j))**2) <= 1.e-6) then
            num_spts = num_spts + 1
            exit
          end if
        end do
    ...
    if (bands_kpt_explicit) then
      ...
      if (ierr /= 0) call io_error('Error in allocating label_idx_special_points in plot_interpolate_bands')

      plot_kpoint(:, :) = bands_kpt_frac(:, :)
      xval = 0.0_dp
    ...
    if (bands_kpt_explicit) then
      do loop_spts = 1, num_spts
        write (bndunit, '(a,3x,I10,3x,4f18.10)') &
          bands_label(label_idx_special_points(loop_spts)), &
...
!~[aam] check divide by ndegen?
    ...
    if (allocated(label_idx_special_points)) deallocate (label_idx_special_points, stat=ierr)
    if (ierr /= 0) call io_error('Error in deallocating label_idx_special_points in plot_interpolate_bands')
  ...
  contains
    ...
    subroutine plot_interpolate_gnuplot
      ...
      if (bands_kpt_explicit) then
        do i = 1, num_spts
          glabel(i) = TRIM(bands_label(label_idx_special_points(i)))
        end do
      else
...
705   format('set arrow from ', F8.5, ',', F10.5, ' to ', F8.5, ',', F10.5, ' nohead')
    ...
    subroutine plot_interpolate_xmgrace
      ...
      if (bands_kpt_explicit) call io_error("bands_kpt_explicit not implemented with xmgrace format")
