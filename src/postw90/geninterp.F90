...
module w90_geninterp
  ...
  use w90_get_oper, only: get_HH_R, HH_R, get_HA_R, HA_R
  use w90_comms
  ...
  private
  public :: geninterp_main, geninterp_hamiltonian
...
107 call io_error('Error: Problem opening output file '//trim(outdat_filename))
  ...
  subroutine geninterp_hamiltonian()
    !! This routine interpolate Hamiltonian matrices, e.g. kinetic, local potential,
    !! non-local potential operators on a dense kmesh.
    !!
    !! This routine is parallel, even if ***the scaling is very bad*** since at the moment
    !! everything must be written by the root node (we need that the output is sorted).
    !! But at least if works independently of the number of processors.
    ...
    integer, dimension(:), allocatable              :: kpointidx, localkpointidx
    real(kind=dp), dimension(:, :), allocatable      :: kpoints, localkpoints
    complex(kind=dp), dimension(:, :), allocatable   :: HA
    complex(kind=dp), dimension(:, :), allocatable   :: UU
    real(kind=dp), dimension(3)                     :: kpt, frac
    complex(kind=dp), dimension(:, :, :), allocatable    :: localHA
    complex(kind=dp), dimension(:, :, :), allocatable    :: globalHA
    logical                                         :: absoluteCoords
    character(len=200)                              :: outdat_filename
    ...
    if (on_root .and. (timing_level > 0)) call io_stopwatch('geninterp_hmn', 1)

    if (.not. geninterp_single_file) call io_error('only support geninterp_single_file = .true.')

    if (on_root) then
      write (stdout, *)
      write (stdout, '(1x,a)') '*---------------------------------------------------------------------------*'
      write (stdout, '(1x,a)') '|                      Generic Hamiltonian Interpolation routines           |'
      write (stdout, '(1x,a)') '*---------------------------------------------------------------------------*'
    ...
    allocate (HA(num_wann, num_wann), stat=ierr)
    if (ierr /= 0) call io_error('Error in allocating HH in geninterp_hmn')
    allocate (UU(num_wann, num_wann), stat=ierr)
    if (ierr /= 0) call io_error('Error in allocating UU in geninterp_hmn')
    ...
    if (on_root) then
      allocate (kpointidx(num_kpts), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating kpointidx in geninterp_hmn.')
      allocate (kpoints(3, num_kpts), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating kpoints in geninterp_hmn.')
      if (geninterp_single_file) then
        allocate (globalHA(num_wann, num_wann, num_kpts), stat=ierr)
        if (ierr /= 0) call io_error('Error allocating globaleig in geninterp_hmn.')
      end if
    else
      ! On the other nodes, I still allocate them with size 1 to avoid
      ! that some compilers still try to access the memory
      ...
      if (ierr /= 0) call io_error('Error allocating kpoints in geninterp_hmn.')
      if (geninterp_single_file) then
        allocate (globalHA(num_wann, num_wann, 1), stat=ierr)
        if (ierr /= 0) call io_error('Error allocating globaleig in geninterp_hmn.')
      end if
    end if
    ...
    allocate (localkpoints(3, max(1, counts(my_node_id))), stat=ierr)
    if (ierr /= 0) call io_error('Error allocating localkpoints in geninterp_hmn.')

    allocate (localHA(num_wann, num_wann, max(1, counts(my_node_id))), stat=ierr)
    if (ierr /= 0) call io_error('Error allocating localeig in geninterp_hmn.')
    ...
    if (.not. geninterp_single_file) then
      ! Allocate at least one entry, even if we don't use it
      allocate (localkpointidx(max(1, counts(my_node_id))), stat=ierr)
      if (ierr /= 0) call io_error('Error allocating localkpointidx in geinterp_hmn.')
      call comms_scatterv(localkpointidx, counts(my_node_id), kpointidx, counts, displs)
    end if
    ...
    if (geninterp_single_file) then
      if (on_root) then
        outdat_filename = trim(seedname)//'_geninterp.hmn'
        outdat_unit = io_file_unit()
        open (unit=outdat_unit, file=trim(outdat_filename), form='formatted', err=107)

        ! call internal_write_header(outdat_unit, commentline)
        write (outdat_unit, '(a)') "# Input file comment: "//trim(commentline)
        write (outdat_unit, '(1x,i10,1x,i10)') num_wann, num_kpts
      end if
    else
      if (num_nodes > 99999) then
        write (outdat_filename, '(a,a,I0,a)') trim(seedname), '_geninterp_', my_node_id, '.hmn'
      else
        write (outdat_filename, '(a,a,I5.5,a)') trim(seedname), '_geninterp_', my_node_id, '.hmn'
      endif
      outdat_unit = io_file_unit()
      open (unit=outdat_unit, file=trim(outdat_filename), form='formatted', err=107)
    ...
    do i = 1, counts(my_node_id)
      kpt = localkpoints(:, i)
      call pw90common_fourier_R_to_k(kpt, HA_R, HA, 0)
      localHA(:, :, i) = HA
      ! call utility_diagonalize(HA, num_wann, localeig(:, i), UU)
    end do

    if (geninterp_single_file) then
      ! Now, I get the results from the different nodes
      call comms_gatherv(localHA, num_wann*num_wann*counts(my_node_id), globalHA, &
      ...
      if (on_root) then
        do i = 1, num_kpts
          ...
          ! I print each line
          ! do enidx = 1, num_wann
          !   write (outdat_unit, '(I10,4G18.10)') kpointidx(i), frac, globalHA(enidx, i)
          ! end do
          do enidx = 1, num_wann
            do j = 1, enidx
              write (outdat_unit, '(2ES20.10)') globalHA(j, enidx, i)
            end do
          end do
        end do
        close (outdat_unit)
      ...
      ! Each node simply writes to its own file
      ! JQ: for the momemnt I do not write kpointidx in output files, to be consistent
      ! with h*mn file format. If each node write to separate files and no kpointidx in output,
      ! it's hard to understand which group of kpoints the output file belongs to,
      ! so I disabled writing to separate files.
      do i = 1, counts(my_node_id)
        ! kpt = localkpoints(:, i)
        ! ! First calculate the absolute coordinates for printing
        ! frac = 0._dp
    ...
    if (allocated(UU)) deallocate (UU)
    if (allocated(localHA)) deallocate (localHA)
    if (allocated(globalHA)) deallocate (globalHA)

    if (on_root .and. (timing_level > 0)) call io_stopwatch('geninterp_hmn', 2)
...
106 call io_error('Error: Problem reading k-point file '//trim(seedname)//'_geninterp.kpt')
107 call io_error('Error: Problem opening output file '//trim(outdat_filename))
  end subroutine geninterp_hamiltonian
